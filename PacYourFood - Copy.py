# myTeam.py
#run the tournament with:  python capture.py -q  -r PacYourFood.py  -b NicNacPacWack.py -l RANDOM
# ---------
...
def is_valid_state():
    for i in gameState.getRedTeamIndices():
        if (gameState.hasWall(gameState.data.agentStates[i].getPosition()) ):
            return False;
...
def team_heuristic(gameState, red, verbose = False):
    ...
    output["return home"] = np.sum(shared.w_dist_home * my_dist_home * (my_carrying)**2);
...
class EnemyTracker: 
    ...
    def update_eaten_agent(self,gameState,index_eaten):
        ...
        self.carrying_food[index_eaten] = 0.0;
...
class DummyAgent(CaptureAgent):
  ...
  def chooseAction(self, gameState):
    ...
    while time.time() < deadline and d <= max_d:
        temp_state = deepcopy(gameState);
        _,action = self.alphaBeta(temp_state,0,d,-10000000,10000000,deadline);     
        if(action != None):
            my_action = action;
            fail=False;
        d += 1;
    ...
    if(my_action == Directions.STOP and global_distancer.getDistance(my_pos,enemy0_pos) >= 3.0 and global_distancer.getDistance(my_pos,enemy1_pos) >= 3.0):
        my_action = np.random.choice(gameState.getLegalActions(self.index));
  ...
  def alphaBeta(self, gameState, depth, maxDepth, alpha,beta, deadline):
    ...
    if gameState.isOver() or gameState.data.timeleft == 0:#game over,terminal state
        ...
        if self.red:
            return 100000*finalScore, Directions.STOP;
        else:
            return -100000*finalScore, Directions.STOP;

    if depth >= maxDepth:
        return (symmetric_heuristic(gameState,self.red), Directions.STOP);
    ...
    if(agentRed == self.red):
        ...
        #try:
        for a,s in actions_states:
            value_action = self.alphaBeta(s,depth+1,maxDepth,alpha,beta,deadline);
            evaluation = value_action[0];
            if(evaluation == None):
                return None, None;
            if(evaluation > v):
                best_action = a;
